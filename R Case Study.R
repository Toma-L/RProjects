...
library(fpc)
# pamk() 的獨特之處在於不需輸入k值
pamk.result <- pamk(iris2) # PAM無法用在大規模的資料集，屆時要改用CLARA
pamk.result$nc
table(pamk.result$pamobject$clustering, iris$Species)
layout(matrix(c(1, 2), 1, 2))
plot(pamk.result$pamobject) # 右圖的si值越大，表示該點被分到相似性較大的cluster中，分類正確，負值代表分類錯誤
layout(matrix(1)) # 畫完圖把作圖版面調整回來
...
library(fpc)
iris2 <- iris[-5]
ds <- dbscan(iris2, eps = .42, MinPts = 5)
table(ds$cluster, iris$Species)
plot(ds, iris2)
plot(ds, iris2[c(1, 4)])

plotcluster(iris2, ds$cluster) # fpc套件提供的分群展示方式


set.seed(435)
idx <- sample(1:nrow(iris), 10)
newData <- iris[idx, -5]
newData <- newData + matrix(runif(10 * 4, min = 0, max = .2))
